...
int main(int argc, char **argv)
{
	try {
		...
		EyeTemplateTEST eyeTemplateTEST;
		...
		fprintf(pf,"MAX_DIST, SIGMA, SMOOTHBLOCKSIZE, SIZEIMAGEDISK,  XY_DISTANCE, X_DISTANCE, Y_DISTANCE, GOOD, BAD\n\n");



		for (float sigma = 501; sigma <= 750; sigma = sigma + 1) {

			for (int SmoothBlockSize = 29; SmoothBlockSize <= 29; SmoothBlockSize = SmoothBlockSize + 2) {

				for (int sizeImageDisk = 5; sizeImageDisk <= 5; sizeImageDisk++) {
					...
					for( boost::filesystem::directory_iterator dataset_image(dataset_path); dataset_image != end_itr; ++dataset_image ) {
						if (counter <652) {
							...
							Point calculatedPoint = *(eyeTemplateTEST.ProcessToExtractFeatures(input_gray, input, sigma, SmoothBlockSize, sizeImageDisk));
					...
					fprintf(pf," %d \t %.0f \t %d \t %d \t %f \t %f \t %f \t %d \t %d \n", MAX_DIST, sigma, SmoothBlockSize, sizeImageDisk, xy_total_dist, x_total_dist, y_total_dist, good, bad);
		...
		if(counter > 0) {
			...
			fprintf(pf,"XY_DISTANCE, X_DISTANCE, Y_DISTANCE\n\n");

			for(int j = 0; j < XY_DISTANCE.size(); j++) {
				fprintf(pf,"%f %f %f\n", XY_DISTANCE[j], X_DISTANCE[j], Y_DISTANCE[j]);
			}
